...
@section('header')
	@if (Auth::check())
		<div class="btn-group pull-right">
			<a href="{{ route('voitures.create') }}" class="btn btn-info">
				Ajouter une voiture
			</a>
	...
	@else
		<a href="{{ url('login') }}" class="btn btn-info pull-right">
			Se connecter
		</a>
	@endif
@endsection
			...
			<div class="col-md-12">
				<header>
					<h1>
						...
						{{ $voiture->couleur }} {{ $voiture->cylindree }}l.
					</h1>
				</header>
				<hr />
				<section>
					...
					@if ((Auth::check() && Auth::user()->admin) || (Auth::check() && Auth::user()->name == $voiture->user->name))
						<form
							method="POST"
							action="{{ route('voitures.destroy', [$voiture->id]) }}"
